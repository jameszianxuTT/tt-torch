...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if self.center:
            dw = dw.astype(float) / 2  # divide padding into 2 sides
            dh = dh.astype(float) / 2

        if shape[::-1] != new_unpad:  # resize
            img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(np.round(dh - 0.1)) if self.center else 0, int(np.round(dh + 0.1))
        left, right = int(np.round(dw - 0.1)) if self.center else 0, int(np.round(dw + 0.1))
